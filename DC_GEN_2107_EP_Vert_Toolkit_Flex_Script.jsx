...
function generateName(){
    ...
    var social = genSocialCode();
    
    name = formatTitle ( name ) + dash( extra ) + dash( date ) + dash( time ) + dash( social );
    return name
}
...
function genSocialCode(){
    if (!myPalette.grp.opt.socialSelected.value){
        return "";
    }
    
    value = String(myPalette.grp.social.social.socialString.text); 
    return value;
}
...
function buildUI(thisObj) {
    ...
    if (myPal != null) {
       ...
       //hackish... turn off the position offset.
        ...
        myPal.day_layer.transform.position.setValue([-18,23.5]);
...
*/
        ...
        myPal.updateBars = function(){
            ...
            if ( myPal.grp.opt.dateSelected.value ){
                ...
                myPal.ctrl_extra_layer("Transform")("Position").setValue(myPal.ctrl_day_layer("Transform")("Position").value + [36-day_width -myPal.dayMargin,myPal.extraVerticalOffset,0]);
            }
            
            if ( ( !myPal.grp.opt.dateSelected.value ) && ( !myPal.grp.opt.timeSelected.value )){
                 myPal.tune_in_layers[0].enabled = myPal.tune_in_layers[1].enabled = false;
            }else{
                myPal.tune_in_layers[0].enabled = myPal.tune_in_layers[1].enabled = true
            }
       ...
       */
        ...
        myPal.grp.time.enabled = myPal.grp.extra.enabled = myPal.grp.social.enabled = myPal.grp.tune_in.enabled = myPal.grp.tune_in.opt.dateSmall.enabled = myPal.grp.tune_in.opt.dayFlat.enabled = myPal.grp.tune_in.opt.dateSmall.enabled = false;
        ...
        */
        ...
        myPal.grp.opt.timeSelected.onClick = function(){
            ...
            if ( !timeSelected ){
                myPal.grp.time.time.timeString.text = "";
            }else{
        ...
        myPal.grp.tune_in.date.dateString.onChanging = myPal.grp.extra.extra.extraString.onChanging = myPal.grp.social.social.socialString.onChanging = myPal.grp.title.title.titleString.onChanging = myPal.grp.time.time.timeString.onChanging = myPal.updateAll;
        /*
        myPal.grp.extra.extra.extraString.onChange = myPal.grp.time.time_slot.timeOption.onChange = myPal.grp.time.time_slot.half.onClick = myPal.grp.time.time_slot.nightTime.onClick = myPal.grp.tunein.day.dayOption.onChange = myPal.grp.tuneindate.day.dateOption.onChange = myPal.grp.tuneindate.day.monthOption.onChange = myPal.updateAll;       
        */

...
function generateName(){
    ...
    var time = genTimeCode () ;
    var avatars = genNumAvatarCode();
    
    name = formatTitle ( name ) + dash( extra ) + dash( date ) + dash( time ) + dash( avatars );
    return name
}
function genExtraCode(){
    ...
    value = myPalette.grp.extra.extra.extraString.text; 
...
function genDateCode(){
    ...
    value = myPalette.grp.tune_in.date.dateString.text;  
...
function genTimeCode(){
    ...
    value = myPalette.grp.time.time.timeString.text;
    
    return value.replace(":",".")
}
function genNumAvatarCode(){
    var numAvatars = myPalette.grp.avatar.avatar.avatarOption.selection.index + 1;

    var code = String(numAvatars)+"xAvatars";
    if ( numAvatars == 1 ){
        code = ""
    }
    return code;
...
function buildUI(thisObj) {
    ...
    if (myPal != null) {
        ...
        */
        ...
        opt: Group { \
                            timeSelected: Checkbox { text: 'Time', alignment: ['left','center']}, \
          ...
          time: Group { \
                    ...
                    time: Group { \
                        ...
                        timeString: EditText {text:'7:30p',alignment: ['fill','center']}, \
                        } \
                    } \
        tune_in: Group { \
                    alignment: ['fill','top'], \
                    ...
                    date: Group { \
                        alignment: ['fill','top'], \
                        alignChildren: ['fill','top'], \
                        dateText: StaticText {text:'Tune In Date', alignment: ['left','center']}, \
                        dateString: EditText {text:'Tonight',alignment: ['fill','center']}, \
                        }, \
                    opt: Group{\
                        alignment: ['fill','top'], \
                        alignChildren: ['fill','top'], \
                        dateSmall: Checkbox {text:'Small', alignment: ['right','center']}, \
                        dayFlat: Checkbox {text:'Flat', alignment: ['right','center']}, \
                        }\
                    } \
        extra: Group { \
                    alignment: ['fill','top'], \
       ...
       myPal.extra_layers  = [
        ...
        myPal.tune_in_layers = [
            myPal.comp.layer("bubble_connect_extra 2"),
            myPal.comp.layer("bubble_date+time_guts"),
            myPal.comp.layer("disney_logo"),
            myPal.comp.layer("bubble_disney_logo"),
            myPal.comp.layer("disney_logo_extra"),
            myPal.comp.layer("bubble_disney_logo_extra")
...
*/
        ...
        myPal.getTuneinNoLogoWidth = function (){
            ...
            var mult = 2.25;
            if ((!myPal.grp.opt.dateSelected.value)&&(myPal.grp.opt.timeSelected.value)){
                mult = 1;
            }
            
            if ( !myPal.grp.opt.extraSelected.value )
            {
                value = time_width + day_width + ( (myPal.timeMargin* mult) );
            }
            else
            {
                value = time_width + day_width + (myPal.timeMargin* mult ) +  extra_width + (myPal.timeMargin*2.60) + connecting_width;
            }
        ...
        myPal.updateBars = function(){
            ...
            if ( myPal.grp.opt.dateSelected.value ){
                ...
                myPal.ctrl_time_layer("Effects")("date_bubble_length")("Slider").setValue(time_width+day_width+(myPal.timeMargin));
            }
            ...
            if( myPal.grp.opt.dateSelected.value ){
                ...
                myPal.ctrl_extra_layer("Transform")("Position").setValue(myPal.ctrl_day_layer("Transform")("Position").value + [37 -day_width -myPal.dayMargin,myPal.extraHorizOffset ,0]);
            }
            ...
            var isDisabled = ( !myPal.grp.opt.dateSelected.value ) && ( !myPal.grp.opt.timeSelected.value );
            for ( var i = 0 ; i < myPal.tune_in_layers.length ; i ++ ){
                myPal.tune_in_layers[i].enabled = !isDisabled;
            }
        ...
        myPal.updateOptions = function(){
            
            if ( ( !myPal.grp.opt.dateSelected.value ) && ( !myPal.grp.opt.timeSelected.value )){
                myPal.grp.extra.enabled = myPal.grp.opt.extraSelected.enabled = myPal.grp.opt.extraSelected.value = false;
            }else{
                myPal.grp.opt.extraSelected.enabled = true;
            }
        }
        myPal.updateAvatarLayers = function(){
         ...
         myPal.updateTime = function(){
          myPal.applyTime( myPal.grp.time.time.timeString.text );
        }
        myPal.updateDate = function(){
            myPal.applyDate(myPal.grp.tune_in.date.dateString.text);
        }
        ...
        myPal.applyDate = function( s ){
            if ( !myPal.grp.opt.dateSelected.value ){ 
                s = "";
            }
            app.beginUndoGroup("applyDate");
        ...
        myPal.updateFlat = function(){
            ...
            if ( myPal.grp.opt.dateSelected.value ){
                isFlat = myPal.grp.tune_in.opt.dayFlat.value;
            }
            else{
                isFlat = false;
            }
        ...
        myPal.updateVisibility = function(){
            var isEnabled = ( myPal.grp.opt.dateSelected.value ) || ( myPal.grp.opt.timeSelected.value );
            if ( isEnabled ){
                ...
                for ( var i = 0 ; i < myPal.extra_layers.length ; i ++ )
                {
                    myPal.extra_layers[i].enabled =  false ;
                }
                for ( var i = 0 ; i < myPal.not_extra_layers.length ; i ++ )
                {
                    myPal.not_extra_layers[i].enabled = false ;
                }
        ...
        myPal.applyTime = function( s ){
            if ( !myPal.grp.opt.timeSelected.value ){ 
                s = "";
            }
            new_string = s;
            var singleton = new_string[s.length-1]!="p";
            if ((singleton)){
                new_string+="[";
            }
            app.beginUndoGroup("applyTime");
            myPal.time_layer("Text")("Animators")("singleton").enabled = singleton;
            myPal.time_layer.sourceText.setValue( new_string );
            app.endUndoGroup();
        };
        ...
        {
            var isFlat = myPal.grp.tune_in.opt.dayFlat.value && myPal.grp.opt.dateSelected.value ;
       ...
       */
        ...
        myPal.grp.time.enabled = myPal.grp.extra.enabled = myPal.grp.tune_in.enabled = myPal.grp.tune_in.opt.dayFlat.enabled = myPal.grp.tune_in.opt.dateSmall.enabled = false;
        
        myPal.lastTime = myPal.grp.time.time.timeString.text;
        myPal.lastDate = myPal.grp.tune_in.date.dateString.text;
        
        //myPal.grp.title.title.isEvent.value = myPal.grp.tuneindate.enabled = myPal.grp.tune_in.opt.dayFlat.enabled = myPal.grp.tunein.day.dayShort.enabled = myPal.grp.extra.enabled = false;
        
        // control possible options

        myPal.grp.opt.dateSelected.onClick = function(){
            var dateSelected = this.value 
            myPal.grp.tune_in.enabled = myPal.grp.tune_in.opt.dayFlat.enabled = myPal.grp.tune_in.opt.dateSmall.enabled = dateSelected; 
            
            if ( !dateSelected ){
                myPal.grp.tune_in.date.dateString.text = "";
            }else{
                myPal.grp.tune_in.date.dateString.text = myPal.lastDate;
            }
            
            myPal.updateAll();
        }
        myPal.grp.opt.timeSelected.onClick = function(){
            var timeSelected = this.value 
            myPal.grp.time.enabled = timeSelected;
            
            if ( !timeSelected ){
                myPal.grp.time.time.timeString.tex0= "";
            }else{
                myPal.grp.time.time.timeString.text = myPal.lastTime;
            }
            myPal.updateAll();
        }
        ...
        myPal.grp.tune_in.opt.dayFlat.onClick = function(){
                        
            myPal.updateAll();
        }
        myPal.grp.tune_in.opt.dateSmall.onClick= function(){
            //writeLn(myPal.grp.tunein.day.dayShort.value);
            myPal.updateAll();
        }
        //Assign Methods to UI
        //Update on every UI change/click
        myPal.grp.avatar.avatar.avatarOption.onChange = myPal.grp.title.title.titleString.onChanging = myPal.grp.extra.extra.extraString.onChanging = myPal.grp.time.time.timeString.onChanging = myPal.grp.tune_in.date.dateString.onChanging = myPal.updateAll;
        //myPal.grp.avatar.avatar.avatarOption.onChange = myPal.grp.tune_in.date.dayShort.onClick = myPal.grp.title.title.titleString.onChanging = myPal.grp.extra.extra.extraString.onChanging = myPal.grp.time.time.timeString.onChanging = myPal.grp.tune_in.date.dateString.onChanging = myPal.updateAll;
